 ...
 * comment for why this is its own item (`RUNEXEC#<id>`, NOT `RUN#<id>#EXEC` — that collided with
 * every `begins_with(sk, "RUN#")` workspace-runs scan and crashed `checkBudget` on every single
 * agent turn, confirmed live) rather than a field on the `Run` row. */
async function recordExecutionArn(workspaceId: string, runId: string, arn: string | undefined): Promise<void> {
  if (!arn) return;
  await ddb.send(
    new PutCommand({ TableName: TABLE_NAME, Item: { pk: `WORKSPACE#${workspaceId}`, sk: `RUNEXEC#${runId}`, durableExecutionArn: arn } }),
  );
}
