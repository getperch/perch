...
import { dispatchMentions } from "./agent-mentions.js";
import { addReaction, appendRunStep, checkBudget, completeRun, createRun, getRecentChannelMessages, listMembers, loadAgentConfig, postMessage } from "./persist.js";
...
import { makeCreateReminderTool } from "./reminder-tool.js";
...
export type AgentMessageRunEvent = {
  ...
  channelTopic?: string;
  prompt: string;
  /** Pre-generated by the caller (services/api/src/routers/messages.ts) so it can persist this
   * run's durable-execution ARN (see services/api/src/run-execution.ts) before `createRun` below
   * even runs — `workflow.start()` only hands the ARN back to the caller, and the caller doesn't
   * otherwise know what id `createRun` would have picked for it. Falls back to a fresh ulid if
   * absent (defensive — every real caller sets it) so this stays optional rather than a breaking
   * change to the event shape. */
  runId?: string;
  /** How many agent-to-agent @mention hops produced this run — 0 (the default, absent) for a run
   * a person triggered directly. Set by agent-mention-events.ts when this run exists because a
   * *different* agent's own reply @mentioned this one (see agent-mentions.ts's `dispatchMentions`)
   * — the mechanism that actually fixes https://github.com/getperch/perch/issues/3's "agents may
   * fail to delegate tasks properly". Capped at `MAX_MENTION_DEPTH` purely to bound a worst-case
   * A-mentions-B-mentions-A ping-pong to a small, fixed number of runs — everything else about a
   * mention-dispatched run (budget cap, approvals) is the same `mode:"direct"` path any other
   * direct run goes through, so those safeguards apply for free without special-casing here. */
  depth?: number;
};
...
/** A short transcript of what else has just happened in this channel — a person's question to a
 * different agent, that agent's answer, another person's follow-up — none of which this agent's
 * own session memory would otherwise contain (see persist.ts's `getRecentChannelMessages` for why
 * that gap exists and why it's the real cause of the "group coordination fails" report this fixes:
 * https://github.com/getperch/perch/issues/3). Excludes the message that triggered this run
 * (that's `event.prompt` already) and anything with no text (a2ui-only cards, approvals). Capped
 * short and to the last 10 lines — this rides along on every turn, so it isn't worth the tokens to
 * make it exhaustive. */
async function recentActivityBlock(workspaceId: string, channelId: string, excludeMessageId: string): Promise<string> {
  const [messages, members] = await Promise.all([getRecentChannelMessages(channelId), listMembers(workspaceId)]);
  const nameById = new Map(members.map((m) => [m.id, m.name]));
  const lines = messages
    .filter((m) => m.id !== excludeMessageId && m.text?.trim())
    .slice(-10)
    .map((m) => `${nameById.get(m.authorId ?? "") ?? "someone"}: ${m.text!.slice(0, 300)}`);
  if (lines.length === 0) return "";
  return (
    "\n\n## Recent activity in this channel\nFor context — you may not have seen this yourself, since it may " +
    `have been directed at someone else:\n${lines.join("\n")}`
  );
}
...
export const handler: ReturnType<typeof workflow.handler> = workflow.handler(async (event: AgentRunEvent, ctx: workflow.Context) => {
  ...
  try {
    ...
    // `create_reminder` is the same kind of standard, always-on capability — see reminder-tool.ts
    // for why an agent needs this rather than telling a person "I can't set reminders, go to
    // Settings → Schedules" for something as ordinary as "remind me to pick that up this
    ...
    const posted = await ctx.step("post-result", () =>
      postMessage({
        workspaceId: event.workspaceId,
        channelId: event.channelId,
        authorId: agent.id,
    ...
    // The other half of this run's fix for github.com/getperch/perch/issues/3 — see
    // recentActivityBlock above for the "can't see what other agents said" half, and
    // agent-mentions.ts's header comment for why this only puts an event on the bus rather than
    // dispatching the mentioned agent directly.
    await ctx.step("dispatch-mentions", () =>
      dispatchMentions({
        workspaceId: event.workspaceId,
        channelId: event.channelId,
        channelName: event.channelName,
        channelTopic: event.channelTopic,
        messageId: posted.id,
        text: response.answer,
