...
function ssmPath(workspaceId: string, agentId: string): string {
  return `/perch/${STAGE}/${workspaceId}/agents/${agentId}/google-workspace-refresh-token`;
}
function clientSsmPath(workspaceId: string): string {
  return `/perch/${STAGE}/${workspaceId}/google-oauth-client`;
}

// Least-privilege Google scope this tool needs — re-derived from `@perch/core`'s
// `GOOGLE_SCOPE_CALENDAR_EVENTS` (same "no shared package with services/tools, keep in sync"
// convention as the SSM paths above). Covers both listing and creating events.
...
const TOKEN_READER_ROLE_ARN = process.env.GOOGLE_TOKEN_READER_ROLE_ARN ?? "";
 ...
 * permission at all: it assumes `GoogleTokenReader` here with an inline session policy scoped to
 * exactly this `(workspaceId, agentId)`'s two parameters. The resulting credentials can read
 * nothing else — see infra/api.ts's `GoogleTokenReader` comment.
 *
...
async function ssmClientForAgent(workspaceId: string, agentId: string): Promise<SSMClient> {
  if (!TOKEN_READER_ROLE_ARN || !ACCOUNT_ID) {
    throw new Error("calendar: GOOGLE_TOKEN_READER_ROLE_ARN / ACCOUNT_ID not configured — check infra/api.ts");
  }
  ...
  const assumed = await sts.send(
    new AssumeRoleCommand({
      ...
      RoleSessionName: `calendar-${agentId}`.slice(0, 64),
  ...
  if (!c?.AccessKeyId || !c.SecretAccessKey || !c.SessionToken) {
    throw new Error("calendar: AssumeRole returned no usable credentials");
  }
 ...
 * whose Google connection predates the calendar tool grant (or was connected for gmail only) won't
 * have `calendar.events`, and should get a clear "reconnect" error rather than an opaque 403 from
 * the Calendar API.
 */
export async function getAccessToken(workspaceId: string, agentId: string, requiredScopes: string[]): Promise<string> {
  ...
  const refreshToken = await getRefreshToken(ssm, workspaceId, agentId).catch((err) => {
    if (err instanceof GoogleWorkspaceNotConnectedError) {
      throw new Error("Calendar isn't connected for this agent — connect Google Workspace from the agent's settings in Perch.");
    }
    throw err;
  });
...
export const CALENDAR_SCOPES = { events: SCOPE_CALENDAR_EVENTS };
