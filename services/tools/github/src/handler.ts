 ...
 * `event` is the flat tool-arguments object plus the reserved `__workspaceId` / `__agentId` /
 * `__runId` keys services/agent-runtime/src/tools.ts injects on every tool call (`__agentId` is
 * what `start_coding_task` uses to run the coding task as the calling agent's own model — see
 * `lookupAgentModel` below). AgentCore's inline tool schema has no enum/oneOf, so this is one
 * `action` string plus flat optional fields — same shape as the gmail/calendar tools.
 */
export const inputSchema = z.object({
  action: z.enum(["get_issue", "create_issue", "close_issue", "open_pull_request", "pr_comment", "start_coding_task"]),
  ...
  prNumber: z.number().int().positive().optional().describe("get_issue / close_issue: the issue number; pr_comment: the pull request number"),
...
const reservedContextSchema = z.object({ workspaceId: z.string(), runId: z.string(), agentId: z.string() });

function extractContext(event: Record<string, unknown>) {
  return reservedContextSchema.parse({ workspaceId: event.__workspaceId, runId: event.__runId, agentId: event.__agentId });
}
...
const ddb = DynamoDBDocumentClient.from(new DynamoDBClient({ region: process.env.HOME_REGION }));
...
/** The model the agent that called `start_coding_task` is itself configured with (its own
 * AgentDetailScreen model picker — `config.model`) — deliberately NOT a separate workspace-level
 * "coding agent model" setting. That was tried and reverted: a second, mostly-redundant model
 * picker just to feed one internal Lambda was one setting surface too many, and the calling
 * agent's own choice is already exactly what a person would expect "the agent" to use. Absent (not
 * thrown) on any read failure — the coding-agent Lambda falls back to its own env-var default
 * either way, so a lookup hiccup here should never be why a coding task fails to even start. */
async function lookupAgentModel(workspaceId: string, agentId: string): Promise<string | undefined> {
  try {
    const res = await ddb.send(new GetCommand({ TableName: TABLE_NAME, Key: { pk: `WORKSPACE#${workspaceId}`, sk: `MEMBER#${agentId}` } }));
    const model = res.Item?.member?.config?.model;
    return typeof model === "string" && model.trim() ? model : undefined;
  } catch (err) {
    console.error("github: couldn't read the calling agent's model, falling back to the default:", err instanceof Error ? err.message : err);
    return undefined;
  }
}
...
export const handler = async (rawEvent: unknown) => {
  ...
  const { workspaceId, runId, agentId } = extractContext(event);
  ...
  try {
    switch (input.action) {
      // Reads an existing issue back — a person or another agent will often just say "pick up
      // issue #3" without repeating what it's actually about, and until this existed the only
      // context an agent had was whatever got mentioned in chat (see
      // services/agent-runtime/src/handler.ts's `recentActivityBlock`) — enough to know an issue
      // exists and its URL, never enough to know what it says. Confirmed live: an agent asked to
      // "pick up issue 3" correctly reasoned "I can't fetch issue details directly from GitHub"
      // and stalled asking a person to paste them instead.
      case "get_issue": {
        const repo = need(input, "repo", input.action);
        const number = need(input, "prNumber", input.action);
        if (!owner) throw new Error('"get_issue" needs an owner');
        const issue = (await callGithub(token, `/repos/${owner}/${repo}/issues/${number}`)) as {
          number?: number;
          title?: string;
          body?: string | null;
          state?: string;
      ...
      case "close_issue": {
        const repo = need(input, "repo", input.action);
        const number = need(input, "prNumber", input.action);
        if (!owner) throw new Error('"close_issue" needs an owner');
        const issue = (await callGithub(token, `/repos/${owner}/${repo}/issues/${number}`, {
          method: "PATCH",
          body: JSON.stringify({ state: "closed" }),
        })) as { number?: number; state?: string; html_url?: string };
        return { ok: true, issueNumber: issue.number, state: issue.state, url: issue.html_url };
      }
      ...
      case "start_coding_task": {
        ...
        const modelId = await lookupAgentModel(workspaceId, agentId);
        const payload: CodingTaskPayload = { workspaceId, runId, owner, repo, branch, baseBranch: input.baseBranch, instructions, modelId };
