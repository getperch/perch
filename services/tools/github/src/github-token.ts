 ...
 * services/tools/gmail/src/google-token.ts).
 *
 ...
 * policy scoped to exactly this one parameter, so this shared Lambda holds no standing SSM grant
 * (see infra/api.ts's `ConnectorTokenReader` comment and services/tools/gmail/src/google-token.ts).
 */
...
async function ssmClientForWorkspace(workspaceId: string): Promise<SSMClient> {
  if (!TOKEN_READER_ROLE_ARN || !ACCOUNT_ID) {
    throw new Error("github: CONNECTOR_TOKEN_READER_ROLE_ARN / ACCOUNT_ID not configured — check infra/api.ts");
  }
  ...
  const assumed = await sts.send(
    new AssumeRoleCommand({
      ...
      RoleSessionName: `github-${workspaceId}`.slice(0, 64),
  ...
  if (!c?.AccessKeyId || !c.SecretAccessKey || !c.SessionToken) {
    throw new Error("github: AssumeRole returned no usable credentials");
  }
