...
/** No shared model resolver with services/agent-runtime/src/model.ts (this agent always runs one
 * fixed model rather than a per-workspace-agent choice) — set explicitly in infra/api.ts. */
const MODEL_ID = process.env.CODING_AGENT_MODEL_ID ?? "";
...
export const handler = async (rawEvent: unknown) => {
  ...
  try {
    ...
    const agent = new Agent({
      ...
      model: new BedrockModel({ modelId: MODEL_ID }),
      systemPrompt:
