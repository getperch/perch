...
export async function syncAgentSchedules(workspaceId: string, agent: AgentMember): Promise<void> {
  ...
  const wanted = new Map(
    scheduleRows.filter((r) => r.trigger.enabled && (r.trigger.schedule || r.trigger.runAt) && r.channelId).map((r) => [scheduleName(agent.id, r.index), r]),
  );
  ...
  for (const [name, row] of wanted) {
    const params = {
      ...
      ScheduleExpression: row.trigger.runAt ? `at(${row.trigger.runAt})` : fiveFieldToEventBridgeCron(row.trigger.schedule!),
      // A one-time (`runAt`) schedule should vanish from EventBridge once it's fired rather than
      // sit around forever in `COMPLETED` state — recurring `cron()` schedules keep firing, so
      // this must stay `NONE` for those.
      ActionAfterCompletion: row.trigger.runAt ? ("DELETE" as const) : ("NONE" as const),
