...
/// client id being baked into this binary at compile time — see `routers/google-workspace.ts`'s
/// `/client-id` endpoint. Returns a clear error (surfaced by `call`'s non-2xx handling) if a
/// workspace admin hasn't configured the Google OAuth client in Settings → Integrations yet.
#[tauri::command]
pub async fn google_workspace_client_id(app: AppHandle) -> Result<GetGoogleWorkspaceClientIdResponse, String> {
    call(&app, Method::GET, "/google-workspace/client-id", None::<&()>).await
}
