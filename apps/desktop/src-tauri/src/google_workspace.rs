...
/// `api/google_workspace.rs`'s `google_workspace_authorize`) rather than hardcoding either — this
/// workspace has no real client id until a workspace admin configures it via Settings →
/// Integrations (`google_workspace_save_client`), and the scope set depends on which tools this
