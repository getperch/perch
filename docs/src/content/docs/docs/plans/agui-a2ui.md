---
title: "Plan: AG-UI / A2UI as a standard capability"
description: "Every agent can render and drive interactive UI in the chat, not just post text."
---
...
## Status — implemented on branch `feat/agent-a2ui` (not merged, not committed)

All of Phases 1–5 landed as a first cut. `pnpm typecheck` (13/13), `pnpm test` (28 tests, new
Vitest setup), `pnpm build`, and `cargo check` on the desktop crate all pass.

- **Phase 1 — contract.** `packages/core/src/a2ui.ts`: closed 12-component catalog (11 read-only
  + `Button`), `a2uiDocument` zod (id-uniqueness / ref-integrity / cycle / `https:`-`mailto:`-only
  URL checks), `a2uiCatalogPromptText()`, and a shallow `a2uiCard` wire form. `message.a2ui`
  added. Lives in `@perch/core`, not `api-contract` (dependency direction).
- **Phase 2 — agent emit.** `services/agent-runtime/src/a2ui.ts`: `render_ui` Strands tool +
  `A2UI_INSTRUCTIONS`. `persist.ts` `attachA2ui()` upserts via a deterministic
  `RUN#<id> / A2UI#<toolUseId>` pointer (replay-safe). Wired into **both** `handler.ts` and
  `scheduled.ts`, gated on `config.ui.enabled` (absent = on).
- **Phase 3 — renderer.** `packages/ui/src/a2ui/A2uiBlock.tsx` on Perch tokens. Re-parses the
  shallow card against strict `a2uiDocument`; malformed / newer-version → one-line fallback,
  never throws. Rendered in `ChatScreen` `MessageRow`.
- **Phase 4 — interactivity (not waiting for v1.0-RC).** `Button` **and** `Form`/`Field` in our
  own closed catalog (catalog version now **3**). `POST /channels/{id}/a2ui-actions` (in
  `messages.ts` router) re-validates the action against the card's declared `Button`/`Form`
  (form field values checked against declared `Field`s — unknown dropped, `required` enforced),
  posts a user message carrying a structured `message.a2uiAction` `{ sourceMessageId, actionId,
  label, value }` (rendered as a `⚡ <label>` chip) whose `text` is the agent-facing prompt, and
  starts a follow-up `direct` turn for the authoring agent — reuses the whole existing agent
  path, no new runtime code. Threaded through desktop: `api-client` → App → `ChatScreen` →
  `A2uiBlock` `onAction` (carries `formData`), plus a Tauri command (`messages_a2ui_action`) and
  OpenAPI regen. Shared state / exact A2UI v1.0 RPC alignment still TODO.
- **Phase 5 — standard offering.** `config.ui.enabled` (default true) + a "Chat UI cards" On/Off
  toggle on the agent detail screen. `docs/a2ui.md` written. Catalog is versioned.

### Notes / rough edges

- No stream-schema change was needed — `message.updated` already carries the whole message.
- `render_ui` calls show up as `tool_call` run steps automatically (via
  `ApprovalInterventionHandler.afterToolCall`).
- Vitest was added at the repo root (`vitest.config.ts`, `pnpm test`) — the repo had no test
  runner before. `react-dom` added as a root dev-dep for `renderToStaticMarkup` in UI tests.
- `Form` submit is a single round-trip (submit → follow-up turn); no inline validation feedback
  from the agent yet, and the form locks after one submit.
- `render_ui` takes `{ card, updateKey? }`. `updateKey` gives the agent a **living card** keyed
  `CHANNEL#<id> / A2UIKEY#<agentId>#<updateKey>` that updates in place across turns and action
  runs; keyless calls stay per-run/replay-safe. No cross-agent sharing of a keyed card.
- This branch is shared — concurrent work (a "mentions read state" feature: `Titlebar.tsx`,
  `mention-reads.ts`, `MentionsScreen.tsx`) is also uncommitted here.
