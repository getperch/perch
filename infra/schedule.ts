...
export function makeRoutineScheduling(args: { table: sst.aws.Dynamo; bus: sst.aws.Bus; agentRuntime: sst.aws.Workflow }) {
  ...
  // Explicit, deterministic names on this function and the role below: `agentRuntime` needs both
  // ARNs for its own `create_reminder` tool (services/agent-runtime/src/reminders.ts) to build an
  // EventBridge Scheduler target, but `agentRuntime` has to be constructed *before*
  // `makeRoutineScheduling` runs (`procedureScheduler` links `agentRuntime`, so it can't exist
  // first — a genuine circular resource dependency either way). An SST-generated name would only
  // be known once this Function actually exists; a name fixed here lets infra/api.ts compute the
  // same ARN as a plain interpolated string ahead of time, with no dependency on this resource
  // object at all. Must match infra/api.ts's `procedureSchedulerArn`/`routineSchedulerRoleArn`
  ...
  const procedureScheduler = new sst.aws.Function("ProcedureScheduler", {
    ...
    name: `perch-${$app.stage}-procedure-scheduler`,
  ...
  // one scheduler Lambda. Explicit name for the same reason as `procedureScheduler` above.
  const schedulerRole = new aws.iam.Role("RoutineSchedulerInvokeRole", {
    name: `perch-${$app.stage}-routine-scheduler-invoke`,
