  ...
  properties: [
    {
      ...
      type: "string",
      required: true,
      description: 'One of "get_issue", "create_issue", "close_issue", "open_pull_request", "pr_comment", "start_coding_task"',
    },
    ...
    { name: "prNumber", type: "integer", description: "get_issue / close_issue: the issue number; pr_comment: the pull request number" },
...
export function makeGateway(args: {
  ...
  makeTarget(
    ...
    "github",
    toolGithub.arn,
    "Work with a GitHub repository: look up an existing issue's title/body/state, create or close an issue, " +
      "comment on a pull request, or hand off a coding task (clone, edit, test, commit, push, open a PR) to a " +
      "separate coding agent that reports back in this channel when it's done. Use get_issue whenever an issue is " +
      "referenced by number or URL without its details already given — never ask a person to re-paste what an " +
      "issue says when you can just look it up. Uses the workspace's configured GitHub access token; issues/PRs " +
      "are attributed to its owner. When you get a result back from get_issue, create_issue, close_issue, " +
      "open_pull_request, or pr_comment, don't describe it in a paragraph — render it as an A2UI card: a Card " +
      "with a short title, KeyValue rows for the fields that matter (issue/PR number, repo, state), a " +
      "StatusBadge for open/closed, and a Link component (never a bare KeyValue row) for the URL. Say only a " +
      "brief line above the card, e.g. \"Created!\" or \"Closed.\" — the card carries the details, not your reply.",
    githubInputSchema,
  );
