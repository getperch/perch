...
export function makeApi(args: {
    ...
    "ToolGithubCodingAgent",
    {
      ...
      environment: {
        variables: {
          ...
          // Was left as "" — confirmed live as the cause of a coding task's failure message
          // ("Truncated event message received. Deserialization error...", the SDK's generic
          // error for a Converse/ConverseStream call that never got a valid response): an empty
          // model id is not a request Bedrock can serve. Matches the model id this workspace's own
          // agents (Jed, Rich) already run successfully — same standard `bedrock:Converse`/
          // `ConverseStream` grant below covers it, no `bedrock-mantle:*` needed for this one.
          CODING_AGENT_MODEL_ID: "moonshotai.kimi-k2.5",
        },
      },
    },
  ...
  const toolGithub = new sst.aws.Function(
    "ToolGithub",
    {
      ...
      // `link: [table]` is new here purely so `start_coding_task` can read the workspace's chosen
      // coding-agent model (Settings → General — see packages/core/src/workspace.ts's
      // `codingAgentModel`) before kicking off the coding-agent Lambda; every other action on this
      // tool still never touches the table.
      link: [table],
      environment: {
        ...toolConnectorEnv,
        WORKSPACE_TABLE_NAME: table.name,
  ...
  // Fixes github.com/getperch/perch/issues/3's "agents may fail to delegate tasks properly": an
  // agent's own reply @mentioning another agent used to do nothing (only a person's message ever
  // got dispatched — see services/api/src/routers/messages.ts). agent-mentions.ts (called from
  // agentRuntime's own handler.ts after posting a reply) puts one of these events per mentioned
  // agent on the bus; this subscription is the actual `workflow.start()` call, split into its own
  // Lambda so it can `link: [agentRuntime]` normally instead of `agentRuntime` needing to link to
  // itself (the same self-reference problem `routineScheduleGroupName`, above, already documents).
  bus.subscribe(
    "AgentMentionDispatchSubscription",
    {
      handler: "services/agent-runtime/src/agent-mention-events.handler",
      link: [agentRuntime],
    },
    { pattern: { source: ["workspace.agent-mentions"], detailType: ["agent.mentioned"] } },
  );
