...
export function makeApi(args: {
  ...
  const googleWorkspaceSsmArnPattern = $interpolate`arn:aws:ssm:${region}:${accountId}:parameter/perch/${$app.stage}/*/agents/*/google-workspace-refresh-token`;
  ...
  const googleOAuthClientSsmArnPattern = $interpolate`arn:aws:ssm:${region}:${accountId}:parameter/perch/${$app.stage}/*/google-oauth-client`;

  // -- Per-agent isolation of the Google refresh-token read --------------------------------------
  //
  ...
  // `.../agents/*/google-workspace-refresh-token` pattern would let any one tool call read any
  // agent's token. Static IAM on a shared role can't say "only for the agent this invocation is
  // for", so instead the tool Lambdas hold NO SSM permission directly: each invocation calls
  // `sts:AssumeRole` on this `GoogleTokenReader` role, passing an inline session policy scoped to
  // exactly the one parameter ARN for the `__agentId` on the event (see
  // services/tools/gmail/src/google-token.ts). The base role below can read the whole family; the
  // per-call session policy is what narrows each actual read to a single agent.
  //
  ...
  const googleTokenReaderRole = new aws.iam.Role("GoogleTokenReader", {
    assumeRolePolicy: $jsonStringify({
      Version: "2012-10-17",
      Statement: [{ Effect: "Allow", Principal: { AWS: $interpolate`arn:aws:iam::${accountId}:root` }, Action: "sts:AssumeRole" }],
    }),
  });
  new aws.iam.RolePolicy("GoogleTokenReaderPolicy", {
    role: googleTokenReaderRole.id,
    policy: $jsonStringify({
      Version: "2012-10-17",
      Statement: [{ Effect: "Allow", Action: ["ssm:GetParameter"], Resource: [googleWorkspaceSsmArnPattern, googleOAuthClientSsmArnPattern] }],
    }),
  });
  ...
  // must match services/api/src/google-oauth.ts's path convention exactly.
  // No direct SSM grant — the token read goes through `GoogleTokenReader` with a per-invocation
  ...
  // exact parameter ARN for that session policy; `GOOGLE_TOKEN_READER_ROLE_ARN` is the role to
  // assume.
  const toolGoogleEnv = {
    STAGE: $app.stage,
    HOME_REGION: region,
    ACCOUNT_ID: accountId,
    GOOGLE_TOKEN_READER_ROLE_ARN: googleTokenReaderRole.arn,
  };
  const toolGmail = new sst.aws.Function(
    "ToolGmail",
    {
      handler: "services/tools/gmail/src/handler.handler",
      environment: toolGoogleEnv,
      permissions: [{ actions: ["sts:AssumeRole"], resources: [googleTokenReaderRole.arn] }],
    },
    { provider: toolsProvider },
  );
