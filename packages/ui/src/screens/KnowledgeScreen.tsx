...
export function KnowledgeScreen({
  ...
  isNarrow?: boolean;
  onOpenSidebar?: () => void;
  /** Set when arriving here from an agent's own profile ("View knowledge") — `concepts` has
   * already been filtered to just that agent's observations by the caller (see
   * apps/desktop/src/App.tsx). This just renders the "Showing <X>'s observations" banner and the
   * link back to the unfiltered view. */
  filterLabel?: string;
  onClearFilter?: () => void;
}) {
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <header style={{ height: 56, flex: "none", display: "flex", alignItems: "center", gap: 12, padding: "0 18px", borderBottom: `1px solid ${color.borderLight}` }}>
        ...
        {filterLabel && (
          <>
            <span style={{ width: 1, height: 18, background: color.borderStrong }} />
            <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: color.mutedDark, whiteSpace: "nowrap" }}>
              Showing {filterLabel}
              {onClearFilter && (
                <button onClick={onClearFilter} className="ws-hoverable" style={{ border: "none", background: "none", padding: 0, font: `500 12.5px ${font.sans}`, color: color.accentText, cursor: "pointer", textDecoration: "underline" }}>
                  Show all
                </button>
              )}
            </span>
        ...
        <div className="ws-sb" style={{ width: isNarrow ? 200 : 264, flex: "none", borderRight: `1px solid ${color.borderLight}`, overflowY: "auto", padding: "10px 8px" }}>
          ...
          {!conceptsLoading && concepts.length === 0 && (
            <div style={{ padding: 16, fontSize: 12.5, color: color.mutedLight, lineHeight: 1.6 }}>
              {filterLabel
                ? `No observations from ${filterLabel} yet — they're added as it works, from real conversations.`
