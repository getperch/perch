...
export const updateAgentPatch = z.object({
  ...
  /** Both set together (see AgentDetailScreen's swatch picker) — a bg with no matching fg would
   * leave the avatar unreadable, so there's no reason to ever patch just one. */
