 ...
 */
export const connectorId = z.enum(["google-workspace", "github"]);
...
/**
 * One fine-grained GitHub access token for the whole workspace — no per-agent connect flow. The
 * `github` tool Lambda (services/tools/github) uses it to clone a repo into a Bedrock AgentCore
 * Code Interpreter sandbox, edit + run tests there, push a branch, and open a PR. Pull requests are
 * attributed to whoever owns the token (typically a bot/machine user), not the individual agent.
 */
const GITHUB: ConnectorDescriptor = {
  id: "github",
  name: "GitHub",
  description:
    "Let a coding agent clone a repository, work on a feature, and open a pull request. Uses one workspace access token — PRs are attributed to its owner.",
  docsUrl: "https://github.com/settings/personal-access-tokens/new",
  configFields: [
    { key: "token", label: "Fine-grained access token", placeholder: "github_pat_…", secret: true },
    { key: "defaultOwner", label: "Default owner (org or user)", placeholder: "acme-inc", secret: false },
  ],
  hasPerAgentConnect: false,
};
